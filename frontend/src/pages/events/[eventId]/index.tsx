...
const EventDetailPage: React.FC = () => {
    ...
    const [proofGenerated, setProofGenerated] = useState<Record<string, boolean>>({});
    const [ticketCredential, setTicketCredential] = useState<TicketCredential | null>(null);
    ...
    const handleQuickLogin = useCallback(async (jwt: string, proof: string) => {
        try {
            if (jwt && proof) {
                ...
                try {
                    // First, attempt to request the ticket credential as normal
                    ticketCredential = await api.eventsEventIdRequestTicketCredentialPost({
                        eventId: eventId as string
                    });
                } catch (error) {
                    console.log('Failed to request new ticket credential, checking for existing one');
                    ...
                    ticketCredential = credentials.find(cred => cred.eventId === eventId);
                }
                ...
                if (ticketCredential) {
                    ...
                    throw new Error('Failed to find or request ticket credential');
                }
    
                // Remove jwt and proof from URL without refreshing the page
                window.history.replaceState({}, document.title, `/events/${eventId}`);
            }
        } catch (error) {
            ...
            if (eventId) {
                try {
                    ...
                    if (jwt && proof) {
                        ...
                        const credentials = await api.userMeTicketCredentialsGet();
                        const eventTicket = credentials.find(cred => cred.eventId === eventId);
                        if (eventTicket) {
                            setHasTicket(true);
    ...
    const handleGenerateProof = async () => {
        ...
        try {
            ...
            handleError(`Failed to generate proof: ${error instanceof Error ? error.message : String(error)}`);
        } finally {
    ...
    if (eventNotFound) return <EventNotFoundMessage>No event found</EventNotFoundMessage>;
                ...
                {eventNotFound ? (
                    ...
                    <>
                        <EventHeader>
                            ...
                            <HostCheckInButton onClick={handleHostCheckIn}>Event Host Check In</HostCheckInButton>
                        </EventHeader>
                        <Separator />
                        <EventDate>
                        ...
                        <ExpandButton onClick={() => setIsDescriptionExpanded(!isDescriptionExpanded)}>
                            {isDescriptionExpanded ? 'Show less' : 'Show more'}
                        </ExpandButton>
                        {!hasTicket ? (
                            <>
                                <RequestCredentialButton onClick={handleRequestCredential}>
                                    Request Credential
                                </RequestCredentialButton>
                            </>
                        ) : (
                            <>
                                <CredentialObtainedMessage>Ticket Credential Obtained 🎉</CredentialObtainedMessage>
                                {!proofGenerated[ticketCredential?.eventId || 'unknown'] ? (
                                    <GenerateProofButton 
                                        onClick={handleGenerateProof} 
                                        disabled={isGeneratingProof[ticketCredential?.eventId || 'unknown']}
                                    >
                                        {isGeneratingProof[ticketCredential?.eventId || 'unknown'] ? (
                                            <LoadingContent>
                                                <LoadingSpinner />
                                    ...
                                    <>
                                        <ProofMessage>
                                            ...
                                            reminder - download QR before the event as internet could be spotty
                                        </ProofMessage>
                                        {qrCodeValue && ticketCredential && (
                                            <QRCodeContainer>
                                                <QRCode value={qrCodeValue[ticketCredential.eventId || 'unknown'] || ''} size={256} />
                                            </QRCodeContainer>
                                        )}
                                    </>
                                )}
